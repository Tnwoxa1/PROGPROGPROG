...
class DES {
public:
    string total_code, total_key;//сюда записываем исходные данные:открытый текст и ключ
    string tempCode_1, tempCode_2;//
    string m_code, code;// в m_code записывается результат шифровки(в битах), его же и выводим
    string L, R, C, D;// правая и лева часть. с и д нужны для получения ключей
    vector<string> K;//сюда записываются ключи
   // DES() {}
    // шифрование
...
//    string cipher;
//    int hello[N];
//    vector<unsigned int> Plaintext; // Открытый текст
//    vector<unsigned int> Ciphertext; // Зашифрованный текст
//    unsigned int n, e, d;
