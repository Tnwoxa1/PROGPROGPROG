...
RSA::Keys RSA::GenerateKeys()
{
    ...
    do {
        e = std::uniform_int_distribution<>(1,phi)(mt);
    } while (GCD(e,phi) != 1);
//    e = 3;

    int d = ExtendedEuclid(e,phi).bezu[0];
...
//        unsigned int dd = 0; auto pt__ = reinterpret_cast<char*>(&dd);
//        *(pt__+0) = encryptedText[var*4 + 0];
//        *(pt__+1) = encryptedText[var*4 + 1];
//        *(pt__+2) = encryptedText[var*4 + 2];
//        *(pt__+3) = encryptedText[var*4 + 3];
...
//    for (int var = 0; var < encryptedText.size(); ++var) {
//        plainText[var] = (char)ExpMOD(encryptedText[var],d,n);
//    }
